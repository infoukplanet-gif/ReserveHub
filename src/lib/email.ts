  ...
  await getResend()?.emails.send({
    ...
    html: `
      <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px 24px; color: #0F172A;">
        <h1 style="font-size: 20px; font-weight: 700; margin: 0 0 8px;">ご予約が確定しました</h1>
        ...
        <div style="background: #F8FAFC; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
          <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
            ...
            <tr><td style="padding: 6px 0; color: #64748B;">メニュー</td><td style="padding: 6px 0;">${menuName}</td></tr>
            ${options.length > 0 ? `<tr><td style="padding: 6px 0; color: #64748B;">オプション</td><td style="padding: 6px 0;">${options.join('、')}</td></tr>` : ''}
            ${staffName ? `<tr><td style="padding: 6px 0; color: #64748B;">担当</td><td style="padding: 6px 0;">${staffName}</td></tr>` : ''}
...
export async function sendReminder(data: ReservationEmail) {
  ...
  await getResend()?.emails.send({
    ...
    html: `
      <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px 24px; color: #0F172A;">
        <h1 style="font-size: 20px; font-weight: 700; margin: 0 0 8px;">${customerName}様、明日のご予約リマインドです</h1>
        <p style="font-size: 14px; color: #64748B; margin: 0 0 24px;">${tenantName}へのご来店をお待ちしております。</p>
...
export async function sendCancellationNotice(data: Pick<ReservationEmail, 'customerEmail' | 'customerName' | 'tenantName' | 'menuName' | 'startsAt'>) {
  await getResend()?.emails.send({
    ...
    html: `
      <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px 24px; color: #0F172A;">
        <h1 style="font-size: 20px; font-weight: 700; margin: 0 0 8px;">ご予約がキャンセルされました</h1>
        ...
        <p style="font-size: 14px; color: #64748B;">またのご予約をお待ちしております。</p>
      </div>
    `,
  })
}
...
export async function sendStaffNotification(staffEmail: string, data: ReservationEmail) {
  await getResend()?.emails.send({
    ...
    html: `
      <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px 24px; color: #0F172A;">
        <h1 style="font-size: 20px; font-weight: 700; margin: 0 0 16px;">新規予約が入りました</h1>
        <div style="background: #F0F9FF; border-radius: 12px; padding: 20px;">
          <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
            <tr><td style="padding: 6px 0; color: #64748B; width: 80px;">顧客名</td><td style="padding: 6px 0; font-weight: 600;">${data.customerName}</td></tr>
            ...
            <tr><td style="padding: 6px 0; color: #64748B;">メニュー</td><td style="padding: 6px 0;">${data.menuName}</td></tr>
